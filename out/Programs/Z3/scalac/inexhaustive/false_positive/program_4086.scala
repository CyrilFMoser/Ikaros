...
val v_b: Int = v_a match{
  ...
  case CC_A(CC_D(), _) => 4 
  case CC_B(CC_A(_, _), CC_A(_, _), CC_E(_, _, _)) => 5 
  case CC_B(CC_B(_, _, _), CC_A(_, _), CC_C(_, _, _)) => 6 
  case CC_B(CC_B(_, _, _), CC_A(_, _), _) => 7 
  case CC_B(_, _, _) => 8 
  case CC_B(CC_A(_, _), _, CC_C(_, _, _)) => 9 
  case CC_B(_, _, CC_E(_, _, _)) => 10 
  case CC_B(CC_B(_, _, _), CC_A(_, _), CC_D()) => 11 
  case CC_B(CC_A(_, _), _, CC_D()) => 12 
  case CC_B(CC_A(_, _), _, CC_E(_, _, _)) => 13 
  case CC_B(_, CC_A(_, _), CC_E(_, _, _)) => 14 
  case CC_B(CC_B(_, _, _), _, CC_C(_, _, _)) => 15 
}
}
