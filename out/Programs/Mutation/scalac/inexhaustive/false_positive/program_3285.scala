...
val v_b: Int = v_a match{
  ...
  case CC_A(CC_A(_, _, _), CC_A(_, _, _), CC_B(_, _, _)) => 6 
  case CC_A(_, CC_A(_, _, _), CC_C(_)) => 7 
  case CC_A(CC_A(_, _, _), CC_A(_, _, _), _) => 8 
  case CC_A(CC_A(_, _, _), _, CC_B(_, _, _)) => 9 
  case CC_A(CC_A(_, _, _), CC_A(_, _, _), CC_A(_, _, _)) => 10 
  case CC_A(CC_A(_, _, _), CC_A(_, _, _), CC_C(_)) => 11 
  case CC_A(_, _, _) => 12 
