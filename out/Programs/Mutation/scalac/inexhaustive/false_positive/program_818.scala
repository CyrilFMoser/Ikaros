...
val v_b: Int = v_a match{
  ...
  case CC_A((0,0), 0, _) => 2 
  case CC_A((0,_), 0, CC_A(_, _, _)) => 3 
  case CC_A((_,_), _, CC_A(_, _, _)) => 4 
  case CC_A((_,0), _, _) => 5 
  case CC_A((0,_), 0, _) => 6 
  case CC_A((_,0), 0, _) => 7 
  case CC_A(_, _, CC_A(_, _, _)) => 8 
  case CC_A((0,_), _, CC_A(_, _, _)) => 9 
  case CC_A(_, 0, CC_A(_, _, _)) => 10 
  case CC_A((0,0), _, _) => 11 
  case CC_A(_, _, _) => 12 
  case CC_A((0,_), _, _) => 13 
}
}
