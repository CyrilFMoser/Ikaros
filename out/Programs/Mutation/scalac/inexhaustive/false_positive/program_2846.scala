...
val v_b: Int = v_a match{
  ...
  case CC_A(_, _, _) => 2 
  case CC_A(CC_A(_, _, _), CC_E(_, _), _) => 3 
  case CC_A(CC_A(_, _, _), CC_D(_, _), _) => 3 
  case CC_A(CC_A(_, _, _), _, CC_B(_)) => 3 
  case CC_A(CC_A(_, _, _), CC_F(_, _), _) => 4 
  case CC_A(_, _, CC_A(_, _, _)) => 5 
  case CC_A(_, _, CC_B(_)) => 6 
  case CC_A(CC_A(_, _, _), CC_E(_, _), CC_A(_, _, _)) => 7 
}
}
