...
val v_b: Int = v_a match{
  ...
  case CC_A(_, CC_B(_, _, _), CC_A(_, _, _)) => 5 
  case CC_A(_, CC_A(_, _, _), CC_B(_, _, _)) => 6 
  case CC_A(_, _, CC_B(_, _, _)) => 7 
  case CC_A(CC_C(_, _, _), CC_B(_, _, _), CC_A(_, _, _)) => 8 
  case CC_A(CC_C(_, _, _), CC_A(_, _, _), CC_B(_, _, _)) => 9 
  case CC_A(CC_C(_, _, _), _, CC_B(_, _, _)) => 10 
  case CC_A(_, _, _) => 11 
  case CC_A(CC_C(_, _, _), CC_A(_, _, _), CC_A(_, _, _)) => 12 
  case CC_A(_, _, CC_A(_, _, _)) => 13 
  case CC_A(CC_C(_, _, _), CC_B(_, _, _), CC_B(_, _, _)) => 14 
  case CC_A(_, CC_B(_, _, _), _) => 15 
  case CC_A(CC_C(_, _, _), CC_A(_, _, _), _) => 16 
}
}
