...
val v_b: Int = v_a match{
  ...
  case CC_A(CC_A(_, CC_F(_, _), _), CC_F(_, _), _) => 5 
  case CC_A(CC_A(_, CC_E(_, _), _), CC_F(_, _), _) => 5 
  case CC_A(CC_A(_, CC_D(_, _), _), CC_F(_, _), _) => 5 
  case CC_A(_, _, CC_A(_, _, _)) => 6 
  case CC_A(_, _, CC_B(_)) => 7 
  case CC_A(CC_A(_, _, _), CC_E(_, _), CC_A(_, _, _)) => 8 
}
}
