...
val v_b: Int = v_a match{
  ...
  case CC_B(_, 12, CC_C(_)) => 2 
  case CC_B(_, _, _) => 3 
  case CC_B(CC_B(_, _, _), 12, CC_B(_, _, _)) => 4 
  case CC_B(CC_C(_), _, CC_B(_, _, _)) => 5 
  case CC_B(CC_C(_), _, _) => 6 
  case CC_B(CC_C(_), _, CC_C(_)) => 7 
  case CC_B(CC_C(_), 12, CC_C(_)) => 8 
  case CC_B(CC_C(_), 12, _) => 9 
  case CC_B(_, 12, CC_B(_, _, _)) => 10 
  case CC_B(CC_B(_, _, _), 12, CC_C(_)) => 11 
  case CC_B(CC_B(_, _, _), 12, _) => 12 
  case CC_C(_) => 13 
}
}
