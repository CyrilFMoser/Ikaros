...
val v_b: Int = v_a match{
  ...
  case CC_A(_, _, _) => 5 
  case CC_A(_, CC_B(), CC_A(_, _, _)) => 6 
  case CC_A(_, CC_C(), CC_B()) => 7 
  case CC_A(_, CC_B(), CC_B()) => 8 
  case CC_A(_, CC_C(), _) => 9 
  case CC_A(_, CC_A(_, _, CC_C()), CC_B()) => 10 
  case CC_A(_, CC_A(_, _, CC_B()), CC_B()) => 10 
  case CC_A(_, CC_A(_, _, CC_A(_, _, _)), CC_B()) => 10 
}
}
