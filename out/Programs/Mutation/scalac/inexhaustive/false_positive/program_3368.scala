...
val v_b: Int = v_a match{
  ...
  case CC_A(CC_A(_, CC_C(_), _), CC_A(_, _, _), CC_A(_, _, _)) => 11 
  case CC_A(CC_A(_, CC_B(_, _, _), _), CC_A(_, _, _), CC_A(_, _, _)) => 11 
  case CC_A(CC_A(_, CC_A(_, _, _), _), CC_A(_, _, _), CC_A(_, _, _)) => 11 
  case CC_A(CC_A(_, _, _), CC_A(_, _, _), CC_C(_)) => 12 
  case CC_A(_, _, _) => 13 
  case CC_A(_, _, CC_A(_, _, _)) => 14 
  case CC_A(_, CC_A(_, _, _), _) => 15 
}
}
