...
val v_b: Int = v_a match{
  case CC_B(CC_C(_), 12, CC_B(CC_C(_), _, _)) => 0 
  case CC_B(CC_C(_), 12, CC_B(CC_B(_, _, _), _, _)) => 0 
  case CC_B(_, _, CC_B(_, _, _)) => 1 
  case CC_B(CC_B(_, _, _), _, CC_B(_, _, _)) => 2 
  case CC_B(_, _, CC_C(_)) => 3 
  case CC_B(_, 12, CC_C(_)) => 4 
  case CC_B(_, _, _) => 5 
  case CC_B(CC_B(_, _, _), 12, CC_B(_, _, _)) => 6 
  case CC_B(CC_C(_), _, CC_B(_, _, _)) => 7 
  case CC_B(CC_C(_), _, _) => 8 
  case CC_B(CC_C(_), _, CC_C(_)) => 9 
  case CC_B(CC_C(_), 12, CC_C(_)) => 10 
  case CC_B(CC_C(_), 12, _) => 11 
  case CC_B(_, 12, CC_B(_, _, _)) => 12 
  case CC_B(CC_B(_, _, _), 12, CC_C(_)) => 13 
  case CC_B(CC_B(_, _, _), 12, _) => 14 
  case CC_C(_) => 15 
}
}
...
//      (T_B Int))
// Mutation information: 
// Deleted Pattern CC_B(_, _, CC_B(_, _, _)) at position 1