...
val v_b: Int = v_a match{
  ...
  case CC_E(_, _, CC_F(_, _, _)) => 5 
  case CC_E(_, CC_C(), CC_D(_, _, _)) => 5 
  case CC_E(_, _, CC_D(_, _, _)) => 5 
  case CC_E(_, CC_C(), CC_E(_, _, _)) => 6 
  case CC_E(_, _, CC_E(_, _, _)) => 7 
  case CC_F(CC_F(_, _, _), _, CC_C()) => 8 
  case CC_F(CC_D(_, _, _), 12, _) => 9 
  case CC_F(CC_E(_, _, _), 12, CC_C()) => 10 
  case CC_F(CC_F(_, _, _), _, _) => 11 
  case CC_F(CC_D(_, _, _), _, CC_C()) => 12 
  case CC_F(_, 12, CC_C()) => 13 
  case CC_F(CC_E(_, _, _), _, CC_C()) => 14 
}
}
