...
hours=1
seconds=900 # instead of 3600 for a full hour

while [ "$new_runtime" -lt $((hours * seconds)) ]; do
    remaining_time=$((hours * seconds - new_runtime))  # Calculate remaining time
    ...
    if [ "$new_runtime" -ge $((hours * seconds)) ]; then
        echo "Reached a total runtime of $hours hours."
        break
    fi
...
# Final message
echo "Script execution finished after $hours hours of runtime."